...
if [[ " ${params_array[*]} " != *" interstate "* ]]; then
  OG_FILE="roads/tl_2013_48_prisecroads.shp"
  FILE="i-35"
  NAME="Texas_US_House_Districts"
  SIMPLIFY="100%"

  ogr2ogr -where "FULLNAME='I- 35'" roads/$FILE.shp $OG_FILE

  mapshaper roads/$FILE.shp name=$NAME -simplify dp $SIMPLIFY -o format=geojson map/geojson/$FILE.geojson
    mapshaper roads/$FILE.shp name=$NAME -simplify dp $SIMPLIFY -o format=topojson map/topojson/$FILE.topojson

  SIMPLIFY="100%"

  mapshaper roads/$FILE.shp name=$NAME -simplify dp $SIMPLIFY -o format=geojson map/geojson/$FILE-simplified.geojson
    mapshaper roads/$FILE.shp name=$NAME -simplify dp $SIMPLIFY -o format=topojson map/topojson/$FILE-simplified.topojson
fi